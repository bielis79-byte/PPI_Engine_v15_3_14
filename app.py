...
st.set_page_config(page_title="Modelo Predictivo de Salud v15.3.14 · segmentación actual / histórica aislada · Nivel 1 + Nivel 2", layout="wide")
...
def save_analysis_snapshot(record_key, record_name, metrics_df, rr_corrected, artifact_mask, config):
    ...
    with sqlite3.connect(LONGITUDINAL_DB_PATH) as con:
        ...
        """, (record_key, record_name, '15.3.14', now, config_json, results_json,
...
def _canonical_patient_rows(catalog_df, patient_id):
    """Devuelve todos los RRi del mismo paciente, corrigiendo alias antiguos de la BD."""
...
def save_patient_analysis_bundle(patient_id, record_keys=None, config=None, include_all_snapshotted=True):
    ...
    with sqlite3.connect(LONGITUDINAL_DB_PATH) as con:
        ...
        """, (bundle_id, patient, display, "15.3.14", _bundle_saved_at, json.dumps(ordered, ensure_ascii=False),
              len(ordered), json.dumps(cfg, ensure_ascii=False, default=str)))
        con.commit()
...
def ensure_patient_bundles_from_existing_history():
    """Migra el historial previo: crea un bundle acumulativo por paciente sin recalcular nada."""
    catalog = list_saved_raw_records()
    ...
    for patient in patients:
        ...
        if rows.empty:
            continue
        # Incluye todos los RRi ya conocidos del paciente. Los que no tengan snapshot se marcan pendientes.
        save_patient_analysis_bundle(patient, rows["record_key"].astype(str).tolist(), config={"migrated": True}, include_all_snapshotted=True)
...
st.title("Modelo Predictivo de Salud v15.3.14 · segmentación actual / histórica aislada · Nivel 1 + Nivel 2")
...
with st.sidebar:
    ...
    if _upload_signature_v1538 and _upload_signature_v1538 != _prev_upload_signature_v1538:
        ...
        # v15.3.14: una carga nueva abre SIEMPRE un espacio de trabajo de segmentación limpio.
    ...
    with st.expander("Historial longitudinal v15.3.14", expanded=True):
...
for rec in records:
    ...
    if rec not in st.session_state.restored_records_v152:
        if _restore_segmentation_v15314:
...
if selected_record not in st.session_state.pending_selections_v1522 and _restore_segmentation_v15314:
...
# v15.3.14: NO crear snapshots automáticamente en la sesión actual.
# El análisis se congela únicamente cuando el usuario pulsa el botón explícito de guardado.
if _history_source_v1538 != "Sesión actual":
    for _rec, _data in record_data.items():
    ...
    with st.expander("Guardar análisis actual en el historial", expanded=False):
        ...
        if st.button("Guardar / actualizar snapshot de este análisis", key="save_snapshot_btn_v1537"):
            ...
            if save_analysis_snapshot(_rksv, _save_snapshot_rec_v1537, records_results.get(_save_snapshot_rec_v1537), _dsv.get('rr'), _dsv.get('artifact_mask'), _cfgsv):
                # Consolidar AHORA, y no antes, la segmentación de trabajo en SQLite.
                _wins_save_v15314 = get_record_windows(global_windows_safe, record_windows_safe, _save_snapshot_rec_v1537, use_independent)
                save_record_segmentation(_save_snapshot_rec_v1537, _dsv.get('filename', _save_snapshot_rec_v1537), _wins_save_v15314)
                _pending_save_v15314 = st.session_state.get('pending_selections_v1522', {}).get(_save_snapshot_rec_v1537)
                if _pending_save_v15314 is not None:
                    save_temporal_selection(
                        _save_snapshot_rec_v1537, _dsv.get('filename', _save_snapshot_rec_v1537),
                        float(_pending_save_v15314[0]), float(_pending_save_v15314[1])
                    )
                try:
                    save_patient_analysis_bundle(
                        infer_patient_id(_save_snapshot_rec_v1537), [_rksv],
                        config={'last_session_version':'15.3.14','explicit_save':True},
                        include_all_snapshotted=True
                    )
                except Exception:
                    pass
                st.success("Análisis guardado. Se han congelado configuración, ventanas y resultados; el histórico anterior solo se actualiza ahora de forma explícita.")
            else:
...
with tab1:
    st.subheader("Segmentación tipo Kubios · ventanas móviles v15.3.14")
    ...
    if loaded_from_history_v152:
        ...
        st.info("Hay registros guardados. Puedes abrirlos desde «Historial longitudinal v15.3.14» sin que interfieran con la segmentación de la sesión actual.")
